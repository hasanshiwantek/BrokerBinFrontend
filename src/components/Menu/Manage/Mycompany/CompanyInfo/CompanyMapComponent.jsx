...
const MapComponent = ({ company }) => {
  ...
  googleMapsApiKey: "AIzaSyBxPohwrm4YVcvGHwJLE2hoQ_ATCfodbzc",
});
  ...
  if (!isLoaded || !companiesArray.length || !companiesArray[0]?.latitude) {
    return (
      <div className="flex justify-center  items-center py-20">
        <div className="animate-spin rounded-full h-12 w-12 border-t-4 border-b-4 border-blue-500"></div>
        <span className="ml-4 text-blue-600 text-lg font-medium"></span>
        ...
        >
          ...
          {selectedCompany && (
            <InfoWindow
              ...
              onCloseClick={() => setSelectedCompany(null)}
              options={{ pixelOffset: window.google && window.google.map ? new window.google.maps.Size(0, -40) : undefined, }}
            >
              <ThemeProvider theme={theme}>
                <Tooltip title="View Company Profile" arrow placement="top">
                </Tooltip>
